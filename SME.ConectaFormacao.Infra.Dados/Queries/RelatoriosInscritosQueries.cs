    ...
    public static class RelatoriosInscritosQueries
    {
        public const string ObterInscritosPorFormacao =
            ...
            SELECT P.ID AS codigoFormacao,
                   ...
                   CF_PUBLICO.NOME AS publicoAlvo, 
            ...
            -- Cargo e função do inscrito
                   LEFT JOIN PUBLIC.CARGO_FUNCAO CF_CARGO ON CF_CARGO.ID = I.CARGO_ID 
                   LEFT JOIN PUBLIC.CARGO_FUNCAO CF_FUNC ON CF_FUNC.ID = I.FUNCAO_ID 
            -- Modalidade / Etapa
                   LEFT JOIN PUBLIC.PROPOSTA_MODALIDADE PM ON PM.PROPOSTA_ID = P.ID AND NOT PM.EXCLUIDO 
            -- Público alvo
                   LEFT JOIN PUBLIC.PROPOSTA_PUBLICO_ALVO PPA ON PPA.PROPOSTA_ID = P.ID AND NOT PPA.EXCLUIDO 
                   LEFT JOIN PUBLIC.CARGO_FUNCAO CF_PUBLICO ON CF_PUBLICO.ID = PPA.CARGO_FUNCAO_ID 
        ...
        public const string QueryOrderbyInscritosPorFormacao =
            ...
            ORDER BY
                ...
                -- 3. ORDENAÇÃO PADRÃO
                P.DATA_REALIZACAO_INICIO ASC,
                P.NOME_FORMACAO ASC,
                P.NUMERO_HOMOLOGACAO ASC,
                U.NOME ASC,
                U.LOGIN ASC
            """;
    }
