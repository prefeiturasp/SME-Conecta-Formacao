    ...
    public class CertificadoCursistaSemRfStrategy(ITemplateService templateService) : CertificadoEstrategiaBase(templateService), ICertificadoCodafGeradorConteudo
    {
        ...
        private static string GerarCorpoCertificado(DadosEmissaoCertificadoCodafDto dados)
        {
            return $@"Certificamos para os devidos fins que o(a) servidor(a), <b>{StringExtensao.FormatarNomePessoa(dados.NomeCompleto)}</b>, 
                        CPF: {dados.Documento.AplicarMascaraCpf()}, participou do {dados.TipoFormacao} <b>{dados.NomeFormacao} </b> 
                   ...
                   com carga horária de {dados.HorasTotais ?? dados.CargaHorariaTotalOutra.ConverterHoraMinutoParaInteiro():00} horas, tendo obtido nota de aproveitamento {dados.ConceitoFinal} 
