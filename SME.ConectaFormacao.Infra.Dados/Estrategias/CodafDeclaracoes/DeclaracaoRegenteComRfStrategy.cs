...
{
    public class DeclaracaoRegenteComRfStrategy(ITemplateService templateService) : DeclaracaoEstrategiaBase(templateService), IDeclaracaoCodafGeradorConteudo
    {
        public string GerarHtml(DadosEmissaoDeclaracaoCodafDto dados)
        {
            var layout = ObterLayoutBase(dados);
            return layout
                .Replace("{{TEXTO_DECLARAÇAO}}", GerarCorpoDeclaracao(dados))
