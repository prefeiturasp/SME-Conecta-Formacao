...
namespace SME.ConectaFormacao.Aplicacao.CasosDeUso.CodafCursosNaoHomologados
{
    public class CasoDeUsoAtualizarCodafCursoNaoHomologado(
        ...
        IContextoAplicacao contextoAplicacao) : CasoDeUsoCodafCursoNaoHomologadoBase(dependencias), ICasoDeUsoAtualizarCodafCursoNaoHomologado
    {
        public async Task<Resultado> ExecutarAsync(CodafCursoNaoHomologadoCadastroDto codafCursoNaoHomologadoCadastroDto, long id)
        {
            ...
            try
            {
                await SalvarDependenciasCodafAsync(codafCursoNaoHomologadoCadastroDto, codafCursoNaoHomologadoExistente);
                transacaoDb.Commit();
