...
{
    public class CasoDeUsoNotificarPareceristasParaReanaliseTestes
    {
        private readonly Mock<IMediator> _mediatorMock;
        private readonly CasoDeUsoNotificarPareceristasParaReanalise _casoDeUso;

        public CasoDeUsoNotificarPareceristasParaReanaliseTestes()
        {
            var mocker = new AutoMocker();
            _mediatorMock = mocker.GetMock<IMediator>();

            _casoDeUso = mocker.CreateInstance<CasoDeUsoNotificarPareceristasParaReanalise>();
        }
        ...
        public async Task DadoSituacaoValida_QuandoExecutar_EntaoDeveGerarNotificacaoERetornarResultado()
        {
            ...
            var param = new MensagemRabbit(JsonSerializer.Serialize(dto));
            var proposta = new SME.ConectaFormacao.Dominio.Entidades.Proposta { Situacao = SituacaoProposta.AguardandoReanalisePeloParecerista };
            ...
            _mediatorMock
                .Setup(m => m.Send(It.IsAny<GerarNotificacaoReanalisePareceristaCommand>(), default))
                .ReturnsAsync(true);
            ...
            _mediatorMock.Verify(m => m.Send(It.Is<GerarNotificacaoReanalisePareceristaCommand>(c => c.Proposta == proposta && c.Pareceristas.Count() == dto.Pareceristas.Count()), default), Times.Once);
        }
        ...
        public async Task DadoSituacaoInvalida_QuandoExecutar_EntaoDeveRetornarFalsoENaoGerarNotificacao()
        {
            ...
            _mediatorMock.Verify(m => m.Send(It.IsAny<GerarNotificacaoReanalisePareceristaCommand>(), default), Times.Never);
        }
    }
}
