...
{
    public class CasoDeUsoSalvarPropostaTutorTestes
    {
        private readonly Mock<IMediator> _mediatorMock;
        private readonly CasoDeUsoSalvarPropostaTutor _sut;
        private readonly Faker _faker;

        public CasoDeUsoSalvarPropostaTutorTestes()
        {
            var mocker = new AutoMocker();
            _mediatorMock = mocker.GetMock<IMediator>();
            _sut = mocker.CreateInstance<CasoDeUsoSalvarPropostaTutor>();
            _faker = new Faker();
        }
        ...
        public async Task DadoParametrosValidos_QuandoExecutar_EntaoDeveEnviarCommandERetornarId()
        {
            ...
            var dto = new PropostaTutorDTO { NomeTutor = _faker.Person.FullName };
            var idEsperado = _faker.Random.Long(1, 100);

            _mediatorMock
                .Setup(m => m.Send(It.Is<SalvarPropostaTutorCommand>(c => c.PropostaId == id && c.PropostaTutorDto == dto), default))
            ...
            _mediatorMock.Verify(m => m.Send(
                It.Is<SalvarPropostaTutorCommand>(c => c.PropostaId == id && c.PropostaTutorDto == dto),
                default), Times.Once);
        }
        ...
        public async Task DadoErroNoMediator_QuandoExecutar_EntaoDevePropagarExcecao()
        {
            ...
            _mediatorMock
                .Setup(m => m.Send(It.IsAny<SalvarPropostaTutorCommand>(), default))
