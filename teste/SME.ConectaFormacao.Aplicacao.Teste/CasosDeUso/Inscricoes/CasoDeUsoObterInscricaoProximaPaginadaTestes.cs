...
{
    public class CasoDeUsoObterInscricaoProximaPaginadaTestes
    {
        private readonly Mock<IMediator> _mediatorMock;
        private readonly Mock<IContextoAplicacao> _contextoAplicacaoMock;
        private readonly Mock<IHttpContextAccessor> _httpContextAccessorMock;
        private readonly CasoDeUsoObterInscricaoProximaPaginada _sut;
        private readonly Faker _faker;

        public CasoDeUsoObterInscricaoProximaPaginadaTestes()
        {
            var mocker = new AutoMocker();
            _mediatorMock = mocker.GetMock<IMediator>();
            _contextoAplicacaoMock = mocker.GetMock<IContextoAplicacao>();
            _httpContextAccessorMock = mocker.GetMock<IHttpContextAccessor>();

            _sut = mocker.CreateInstance<CasoDeUsoObterInscricaoProximaPaginada>();
            _faker = new Faker();
        }

        [Fact]
        public async Task DadoFiltroPreenchido_QuandoExecutar_EntaoDeveMapearTodasPropriedadesEnviarQueryERetornarPaginacao()
        {
            // Arrange
            var filtroDto = new InscricaoProximaFiltroDTO
            {
                CodigoFormacao = _faker.Random.Long(1, 10000),
                NomeFormacao = _faker.Lorem.Sentence(),
                NomeTurma = _faker.Company.CatchPhrase(),
                Situacao = _faker.Random.Int(1, 5),
                DataInscricao = _faker.Date.Past(),
            ...
            _mediatorMock
                .Setup(m => m.Send(It.Is<ObterInscricaoProximaPaginadaQuery>(q =>
                    q.UsuarioId == usuarioLogado.Id &&
                    q.NumeroPagina == _sut.NumeroPagina &&
                    q.NumeroRegistros == _sut.NumeroRegistros &&
                    q.Filtro != null &&
                    q.Filtro.CodigoFormacao == filtroDto.CodigoFormacao &&
                    q.Filtro.NomeFormacao == filtroDto.NomeFormacao &&
                    q.Filtro.NomeTurma == filtroDto.NomeTurma &&
                    q.Filtro.Situacao == filtroDto.Situacao &&
                    q.Filtro.DataInscricao == filtroDto.DataInscricao &&
            ...
            _mediatorMock.Verify(m => m.Send(It.Is<ObterInscricaoProximaPaginadaQuery>(q =>
                q.UsuarioId == usuarioLogado.Id &&
                q.NumeroPagina == _sut.NumeroPagina &&
                q.NumeroRegistros == _sut.NumeroRegistros &&
                q.Filtro != null &&
        ...
        [Fact]
        public async Task DadoFiltroNulo_QuandoExecutar_EntaoDeveEnviarQueryComFiltroVazioERetornarPaginacao()
        {
            // Arrange
            InscricaoProximaFiltroDTO? filtroDto = null;
            var usuarioLogado = new EntidadeUsuario { Id = _faker.Random.Long(1, 1000) };
            var resultadoEsperado = new PaginacaoResultadoDto<InscricaoPaginadaDTO>(new List<InscricaoPaginadaDTO>(), 0, _sut.NumeroRegistros);
            ...
            _mediatorMock
                .Setup(m => m.Send(It.Is<ObterInscricaoProximaPaginadaQuery>(q =>
                    ...
                    q.NumeroRegistros == _sut.NumeroRegistros &&
                    q.Filtro != null &&
                    q.Filtro.CodigoFormacao == null &&
                    q.Filtro.NomeFormacao == null &&
                    q.Filtro.NomeTurma == null &&
                    q.Filtro.Situacao == null &&
                    q.Filtro.DataInscricao == null &&
        ...
        [Fact]
        public async Task DadoFalhaAoObterUsuarioLogado_QuandoExecutar_EntaoDevePropagarExcecao()
        {
            // Arrange
            var filtroDto = new InscricaoProximaFiltroDTO();
            ...
            // Act
            var act = () => _sut.Executar(filtroDto);

            // Assert
            await act.Should().ThrowAsync<InvalidOperationException>().WithMessage(mensagemErro);
            _mediatorMock.Verify(m => m.Send(It.IsAny<ObterInscricaoProximaPaginadaQuery>(), It.IsAny<CancellationToken>()), Times.Never);
        }
    }
}
